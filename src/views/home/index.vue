<template>
  <el-container class="wrap">
    <el-aside width="200px"  class="left-list">
      <div class="logo">
        <img
          src="data:image/png;base64," alt/>
      </div>
      <!-- :default-active:写js表达式需要加上冒号 -->
      <el-menu
      unique-opened
      style="width:100%"
      :default-active="$store.state.Uparticles"
      class="el-menu-vertical-demo"
      text-color="#fff"
      router
      background-color="#353b4e"  
      active-text-color="#ffd04b">
      <el-menu-item index="oo">
          <span>首页</span>
      </el-menu-item>
      <el-submenu index="1">
        <template slot="title">
          <span>内容管理</span>
        </template>
          <el-menu-item index="/publish">发布文章</el-menu-item>
          <el-menu-item index="/article">内容列表</el-menu-item>
          <el-menu-item index="/comments">评论列表</el-menu-item>
          <el-menu-item index="/media">素材管理</el-menu-item>
      </el-submenu>
      <el-submenu index="3">
        <template slot="title">
          <span>粉丝管理</span>
        </template>
          <el-menu-item index="3-1">图文数据</el-menu-item>
          <el-menu-item index="/overview">粉丝概况</el-menu-item>
          <el-menu-item index="3-3">粉丝画像</el-menu-item>
          <el-menu-item index="3-4">粉丝列表</el-menu-item>
      </el-submenu>
      <el-menu-item index="/account">
        <template slot="title">
          <span>账户信息</span>
        </template>
      </el-menu-item>
    </el-menu>
    </el-aside>
    <el-main class="right-list">
        <div class="hei-head">
         <h4>江苏传智播客教育科技股份有限公司</h4>
         <el-dropdown trigger="click" @command="handlCommand" placement="bottom-start">
        <span class="el-dropdown-link">
            <img :src="$store.state.userInfo.photo" alt="" class="icon">     
            {{ $store.state.userInfo.name }}<i class="el-icon-arrow-down el-icon--right"></i>
        </span>
        <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command = "info">个人信息</el-dropdown-item>
            <el-dropdown-item command = "git">git地址</el-dropdown-item>
            <el-dropdown-item command = "logout">退出</el-dropdown-item>
        </el-dropdown-menu>
        </el-dropdown>   
        </div>
       <router-view></router-view>
    </el-main>
  </el-container>
</template>

<script>
export default {
    data() {
        return {
            userInfo:{
                photo:"",
                name:""
            }
        }
    },
    methods: {
        handlCommand(cmd){
            if(cmd == "logout"){
                window.localStorage.removeItem('userInfo')
                this.$router.push('/login')
            }else if(cmd == "git"){
                window.open("http://www.github.com")
            }     
        }
    },
    created() {
        let str = window.localStorage.getItem('userInfo')
        this.userInfo = JSON.parse(str);
    },
};
</script>

<style lang="less">
.wrap {
  height: 100%;

  .left-list {
    background-color: #333a4e;
    overflow: hidden;
   .logo{
       padding: 20px 0;
       background-color: #2e2f32;
       text-align: center;        
       img{
        width: 150px;
    }
   }
  }
}
.right-list{
    background-color: #f2f3f5;
    padding: 0;

    .hei-head{
        background: #fff;
        height: 78px;
        padding: 0 20px;
        display: flex;
        justify-content: space-between;
        align-items: center;

        .icon{
            width: 35px;
            height: 35px;
            border-radius: 50%;
        }
    }
}
</style>